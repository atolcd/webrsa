<?php
	...
	class ProgressBarTask extends Shell
	{
		...
		public function set( $done = null, $doneSize = null, $additionalInfos = '' ) {
			...
			if( $message ) {
				$output = sprintf(
						"%.01f%% %d/%d %s %s [%s>%s]", $perc * 100, $this->done, $this->total, $this->_niceRemaining(), __( 'remaining', true ), str_repeat( "-", $doneSize ), str_repeat( " ", $this->size - $doneSize )
				);
				$width = strlen( $output );
			...
			$this->out( "\r".$message.$output.(($additionalInfos != '') ? ' [ '.$additionalInfos.' ]' : '') );
			flush();
		}
