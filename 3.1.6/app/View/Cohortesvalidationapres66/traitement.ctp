<?php
	$this->pageTitle = 'APRE/ADREs à traiter par la cellule';
...
<?php echo $this->Xform->create( 'Cohortevalidationapre66', array( 'type' => 'post', 'action' => 'traitement', 'id' => 'Search', 'class' => ( ( is_array( $this->request->data ) && !empty( $this->request->data ) ) ? 'folded' : 'unfolded' ) ) );?>
...
<?php if( isset( $cohortevalidationapre66 ) ):?>
    <?php if( is_array( $cohortevalidationapre66 ) && count( $cohortevalidationapre66 ) > 0  ):?>
        <?php echo $this->Form->create( 'TraitementApre', array() );?>
		<?php
        ...
        <?php foreach( $cohortevalidationapre66 as $index => $validationapre ):?>
            <?php
				$innerTable = '<table id="innerTablesearchResults'.$index.'" class="innerTable">
                    ...
                    $array2 = array(
                        $this->Form->input( 'Apre66.'.$index.'.istraite', array( 'label' => false, 'type' => 'checkbox', 'value' => $validationapre['Apre66']['istraite']  ) ).
						...
						$this->Form->input( 'Apre66.'.$index.'.etatdossierapre', array( 'label' => false, 'type' => 'hidden', 'value' => 'TRA' ) ),
