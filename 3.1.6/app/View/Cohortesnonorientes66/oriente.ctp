...
<?php echo $this->Xform->create( 'Cohortenonoriente66', array( 'type' => 'post', 'action' => 'oriente', 'id' => 'Search', 'class' => ( ( is_array( $this->request->data ) && !empty( $this->request->data ) ) ? 'folded' : 'unfolded' ) ) );?>
        ...
        <?php echo $this->Xform->input( 'Search.Nonoriente66.datenotification', array( 'label' => 'Filtrer par date de notification', 'type' => 'checkbox' ) );?>
		<fieldset>
			<legend>Date de notification</legend>
			<?php
				$datenotificationFromSelected = $datenotificationToSelected = array();
				if( !dateComplete( $this->request->data, 'Search.Nonoriente66.datenotification_from' ) ) {
					$datenotificationFromSelected = array( 'selected' => strtotime( '-1 week' ) );
				}
				if( !dateComplete( $this->request->data, 'Search.Nonoriente66.datenotification_to' ) ) {
					$datenotificationToSelected = array( 'selected' => strtotime( 'today' ) );
				}

				echo $this->Xform->input( 'Search.Nonoriente66.datenotification_from', Set::merge( array( 'label' => 'Du (inclus)', 'type' => 'date', 'dateFormat' => 'DMY', 'maxYear' => date( 'Y' ), 'minYear' => date( 'Y' ) - 20 ), $datenotificationFromSelected ) );
