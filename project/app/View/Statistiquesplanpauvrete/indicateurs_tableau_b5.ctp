...
<?php if( !empty( $this->request->data ) ) { ?>
	...
	<?php
		...
		$cells = array();
		$row = 0;
		foreach( $results['Pers'] as $key => $result) {
			if($key == 'delai') {
				$cells[$row] = array(__d('statistiquesplanpauvrete', 'Tableaub5.' . $key ));
				$row++;
				foreach( $results['Pers'][$key] as $keyDelai => $delai) {
					$cells[$row] = $delai;
		...
		$thead = $this->Xhtml->tag(
			...
			$this->Xhtml->tableHeaders(
				array(
					__d( 'statistiquesplanpauvrete', 'Tableau.nbCerNvxEntrant' ) . $annee,
					__d( 'statistiquesplanpauvrete', 'Tableau.jan' ),
					__d( 'statistiquesplanpauvrete', 'Tableau.feb' ),
					__d( 'statistiquesplanpauvrete', 'Tableau.mar' ),
					__d( 'statistiquesplanpauvrete', 'Tableau.apr' ),
		...
		$cells = array();
		$row = 0;
		foreach( $results['NvxEnt'] as $key => $result) {
			if($key == 'delai') {
				$cells[$row] = array(__d('statistiquesplanpauvrete', 'Tableaub5.NvxEnt.' . $key ));
				$row++;
				foreach( $results['NvxEnt'][$key] as $keyDelai => $delai) {
					$cells[$row] = $delai;
					$joursDelais = explode('_', $keyDelai);
					$strDelai = '';
					if($joursDelais[0] === '0') {
			...
			if( isset($cells[$row]) && is_array($cells[$row]) )
				array_unshift($cells[$row], array(__d('statistiquesplanpauvrete', 'Tableaub5.NvxEnt.' . $key ), array('class' => 'text-left')));
			$row++;
		}
		$tbody = $this->Xhtml->tag( 'tbody', $this->Xhtml->tableCells( $cells ) );
