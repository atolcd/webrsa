	...
	foreach( $dossiers[$theme]['liste'] as $i => $dossierep ) {
		...
		echo $this->Xhtml->tableCells(
			array(
				...
				array(
					$this->Form->input( "Decisionnonrespectsanctionep93.{$i}.decision", array( 'type' => 'select', 'options' => $options['Decisionnonrespectsanctionep93']['decision'], 'div' => false, 'label' => false, 'empty' => true, 'value' => isset($dossierep['Passagecommissionep'][0]['Decisionnonrespectsanctionep93'][0]['decision']) ? $dossierep['Passagecommissionep'][0]['Decisionnonrespectsanctionep93'][0]['decision'] : null ) ),
				...
				$this->Form->input( "Decisionnonrespectsanctionep93.{$i}.commentaire", array( 'label' =>false, 'type' => 'textarea', 'value' => isset ($dossierep['Passagecommissionep'][0]['Decisionnonrespectsanctionep93'][0]['commentaire']) ? $dossierep['Passagecommissionep'][0]['Decisionnonrespectsanctionep93'][0]['commentaire'] : null ) ).
				$hiddenFields
			),
			array( 'class' => 'odd' ),
			array( 'class' => 'even' )
