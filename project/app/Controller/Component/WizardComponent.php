<?php
	...
	class WizardComponent extends Component
	{
				...
				if( $this->_validStep( $step ) ) {
					...
					if( !empty( $this->Controller->request->data ) && !isset( $this->Controller->request->data['Previous'] ) ) {
						...
						elseif( $this->autoValidate ) {
							...
							trigger_error( __( 'Process Callback not found. Please create Controller::'.$processCallback ), E_USER_WARNING );
						}
					...
					trigger_error( __( 'Step validation: '.$step.' is not a valid step.' ), E_USER_WARNING );
				}
			}
		...
		public function save() {
			$this->Session->write( "$this->_sessionKey.$this->_currentStep", $this->Controller->request->data );
		}
