...
<tr>
	...
	foreach( $dossiers[$theme]['liste'] as $i => $dossierep ) {
		...
		$idDecision = null;
		$idPassage = null;
		if (isset ($dossierep['Passagecommissionep'][0]['Decisioncontratcomplexeep93'])) {
			$idDecision = Set::classicExtract( $dossierep, "Passagecommissionep.0.Decisioncontratcomplexeep93.{$indexDecision}.id" );
			$idPassage = Set::classicExtract( $dossierep, "Passagecommissionep.0.Decisioncontratcomplexeep93.{$indexDecision}.passagecommissionep_id" );
		}

		$hiddenFields = $this->Form->input( "Decisioncontratcomplexeep93.{$i}.id", array( 'type' => 'hidden', 'value' => $idDecision ) ).
						$this->Form->input( "Decisioncontratcomplexeep93.{$i}.passagecommissionep_id", array( 'type' => 'hidden', 'value' => $idPassage ) ).
