<?php
	/**
	 * Code source de la classe Xpaginator2HelperTest.
	 *
	 * PHP 5.3
	 *
	...
	App::uses( 'Xpaginator2Helper', 'View/Helper' );

	/**
	 * Classe Xpaginator2HelperTest.
	 *
	 * @package app.Test.Case.Model.Behavior
	 */
	class Xpaginator2HelperTest extends CakeTestCase
	{
		/**
		...
		public function setUp() {
			...
			$this->Xpaginator2 = new Xpaginator2Helper( $this->View );
			$this->Apple = ClassRegistry::init( 'Article' );

			$this->Xpaginator2->request = new CakeRequest( null, false );
			$this->Xpaginator2->request->addParams(
				array(
					'controller' => 'articles',
					'action' => 'index',
					'paging' => array(
		...
		public function tearDown() {
			parent::tearDown();
			unset( $this->View, $this->Xpaginator2 );
		}

		/**
		 * Test de la méthode Xpaginator2Helper::sort()
		 *
		 * @return void
		 */
		public function testSort() {
			$result = $this->Xpaginator2->sort( 'Article.title' );
			$expected = '<a href="/index/page:1/sort:Article.title/direction:asc">Article.title</a>';
			$this->assertEquals( $result, $expected, var_export( $result, true ) );
		}

		/**
		 * Test de la méthode Xpaginator2Helper::paginationBlock()
		 *
		 * @return void
		 */
		public function testPaginationBlock() {
			Configure::write( 'Optimisations.Articles_index.progressivePaginate', false );
			$result = $this->Xpaginator2->paginationBlock( 'Article', array() );
			...
			$this->Xpaginator2->request->params['paging']['Article']['pageCount'] = 3;
			$this->Xpaginator2->request->params['paging']['Article']['count'] = 41;
			$result = $this->Xpaginator2->paginationBlock( 'Article', array() );
