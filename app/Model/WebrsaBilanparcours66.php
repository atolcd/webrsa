			...
			// Saisine audition
				...
				if( $success = $this->Bilanparcours66->validates() ) {
					...
					if( $nbOrientstruct == 0 ) {
						...
						if ( $data[$this->Bilanparcours66->alias]['examenaudition'] == 'DOD' ) {
							$nbpassageeplaudition = $this->Bilanparcours66->Defautinsertionep66->Dossierep->Passagecommissionep->find(
								'count',
								array(
									'conditions' => array(
										'Passagecommissionep.dossierep_id IN ('.$this->Bilanparcours66->Defautinsertionep66->Dossierep->sq(
											array(
												'fields' => array( 'dossierseps.id' ),
												'alias' => 'dossierseps',
												'conditions' => array(
							...
							$typesrdvs = $this->Bilanparcours66->Contratinsertion->Personne->Rendezvous->Typerdv->find(
								'all',
								array(
									'conditions' => array(
							...
							foreach( $typesrdvs as $typerdv ) {
								$nbrdvsnonvenu = $this->Bilanparcours66->Contratinsertion->Personne->Rendezvous->find(
									'count',
									array(
										'conditions' => array(
											'Rendezvous.personne_id' => $data[$this->Bilanparcours66->alias]['personne_id'],
							...
							if ( $nbpassageeplaudition >= $nbPosPasEplAud ) {
								$this->Bilanparcours66->invalidate( 'examenaudition', 'Cette personne ne possède pas assez de rendez-vous où elle ne s\'est pas présentée.' );
								return false;
							}
						}
