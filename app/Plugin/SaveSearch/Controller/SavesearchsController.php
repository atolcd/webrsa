	...
	class SavesearchsController extends AppController
	{
		...
		public function save() {
			...
			if (!empty($alreadyExists) && !Hash::get($this->request->data, 'Savesearch.id')) {
				$this->Session->setFlash('Une sauvegarde existe déjà pour les même critères : '.Hash::get($alreadyExists, 'Savesearch.name'), 'flash/error' );
			...
			if (!Hash::get($v, 'Savesearch.name')) {
				$this->Session->setFlash('Le Nom de la recherche doit être renseigné', 'flash/error' );
			...
			if (!Hash::get($v, 'Savesearch.url') 
				...
				$this->Session->setFlash('Une erreur s\'est produite lors de l\'enregistrement.', 'flash/error' );
				$this->redirect($redirectTo);
			}
		...
		protected function _save() {
			...
			if ($this->Savesearch->save($data)) {
				$this->Session->setFlash('Recherche sauvegardée avec succès.', 'flash/success' );
			} else {
				$this->Session->setFlash('Une erreur s\'est produite lors de l\'enregistrement.', 'flash/error' );
			}
		...
		protected function _delete($savesearch_id) {
			if ($this->Savesearch->delete($savesearch_id)) {
				$this->Session->setFlash('Recherche supprimée avec succès.', 'flash/success' );
			} else {
				$this->Session->setFlash('Une erreur s\'est produite lors de la suppression.', 'flash/error' );
			}
