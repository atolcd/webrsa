<?php
	...
	class JavascriptHelper extends AppHelper
	{
			...
			if( $this->useNative ) {
				...
				foreach( $data as $key => $val ) {
					if( is_array( $val ) || is_object( $val ) ) {
						$val = $this->object(
								$val, array_merge( $options, array( 'block' => false, 'prefix' => '', 'postfix' => '' ) )
						);
					}
					else {
						$quoteStrings = (
						...
						$val = $this->_value( $val, $quoteStrings );
					}
					if( !$numeric ) {
						$val = $options['q'].$this->_value( $key, false ).$options['q'].':'.$val;
					}
					$out[] = $val;
				}
		 ...
		 */
		protected function _value( $val, $quoteStrings = true ) {
			switch( true ) {
